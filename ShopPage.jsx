function ShopPage() {
  ...
  const pillTones = {
    'myo-strawberry': 'pink', 'myo-vanilla': 'blue', 'myo-chai': 'ink',
    'en-apex': 'dark', 'en-strawberry': 'dark', 'en-lemonade': 'ink',
  };
    ...
    <>
      ...
      <section style={{ background: 'var(--bm-ink)', padding: '72px 48px 40px', borderBottom: '1.5px solid rgba(255,255,255,0.1)' }} className="bm-shop-header">
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--bm-pink)', marginBottom: 16 }}>
          Pre-launch · {filtered.length} of {SITE_PRODUCTS.length} SKUs
        </div>
        <h1 style={{
            ...
            return (
              <Link key={p.id} to={`/product/${p.id}`} className="bm-product-card" style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 13, letterSpacing: '0.08em', textTransform: 'uppercase', color: ac.price }}>Coming Soon</span>
                  <span style={{ fontFamily: 'var(--font-text)', fontWeight: 700, fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: ac.shop }}>Learn more →</span>
                </div>
              </Link>
            );
          })}
